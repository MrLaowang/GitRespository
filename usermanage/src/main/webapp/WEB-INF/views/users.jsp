...
<body>
    ...
    <table class="easyui-datagrid" id="userList" title="会员列表" 
	       data-options="singleSelect:true,collapsible:true,pagination:true,url:'/rest/user',method:'get',pageSize:5,toolbar:toolbar,pageList:[2,5,10]">
	    <thead>
	        <tr>
	...
	<!-- modal 是否将窗体显示为模式化窗口 -->
<div id="userAdd" class="easyui-window"  title="新增会员" data-options="modal:true,shadow:true,closed:true,iconCls:'icon-save',href:'/rest/user/user-add'" style="width:800px;height:600px;padding:10px;">
        The window content.
</div>
...
var toolbar = [{
    ...
    handler:function(){
    	...
    	if(rows.length == 0 ){
    		$.messager.alert("warning","你还未勾选任何选项","warning");
    		return ;
    	}
    	if(rows.length == 1 ){
    		var id = rows[0].id;
    		$("#userAdd").window("open").panel('refresh');
    	}else{
    		$.messager.alert("warning","只能选择一行","warning");
    		return ;
    	}
    }
},{
    ...
    handler:function(){
    	...
    	$.messager.confirm('确认','确定删除ID为 '+ids+' 的会员吗？',function(r){
    	    if (r){
            	 $.post("/rest/user/delete",{'ids':ids,'_method':'DELETE'}, function(data){
        			if(data.status == 200){
    ...
    handler:function(){
    	/*
    		$("#userList").datagrid("getPager") 获取easyui的分页工具条
    		$("#userList").datagrid("getPager").data("pagination").options; 获取分页对象的options
    	*/
    	var optins = $("#userList").datagrid("getPager").data("pagination").options;
    	var page = optins.pageNumber; //当前页
    	var rows = optins.pageSize; //每页显示的记录条数
    	//创建一个form表单并手动提交
    	$("<form>").attr({
